...
ensure_node_tooling() {
  ...
  if [ -n "$BREW_BIN" ]; then
    echo "[ci_post_clone] Node <${REQUIRED_NODE_MAJOR} or npm missing; installing Node via Homebrew"
  ...
  if [ "$(current_node_major)" -lt "$REQUIRED_NODE_MAJOR" ]; then
    echo "[ci_post_clone] NodeJS >=${REQUIRED_NODE_MAJOR} is required; found $(node -v)" >&2
    echo "[ci_post_clone] PATH=$PATH" >&2
    exit 1
  fi
}
