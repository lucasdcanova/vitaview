...
const LEGACY_VISIBLE_TAG = '[APP_STORE_DEMO_2026]';
const APPOINTMENT_SEED_MARKER = 'seed:appstore-2026';
const DEMO_EMAIL_SUFFIX = '@demo.vitaview.app';
...
const ALLERGY_POOL = [
  ...
  { allergen: 'Camarao', reaction: 'Prurido', severity: 'moderada' },
  { allergen: 'Ibuprofeno', reaction: 'Broncoespasmo', severity: 'grave' },
];

const PATIENT_NOTES = [
  'Paciente em acompanhamento regular.',
  'Retorno para revisao de conduta clinica.',
  'Historico de acompanhamento ambulatorial.',
  'Paciente com boa adesao ao plano terapeutico.',
  'Consulta de monitoramento de sintomas.',
  'Seguimento preventivo com exames periodicos.',
];

const APPOINTMENT_NOTES = [
  'Consulta de rotina.',
  'Retorno para avaliacao de exames.',
  'Revisao de plano terapeutico.',
  'Acompanhamento de sintomas.',
  'Consulta de seguimento clinico.',
  'Orientacoes preventivas e plano de cuidado.',
];
...
async function ensureClinicForDoctor(doctorId) {
...
async function ensureClinicMembership(clinicId, userId, role) {
...
function buildPatientSeeds() {
  return PATIENT_NAMES.map((name, index) => {
    ...
    const notes = PATIENT_NOTES[index % PATIENT_NOTES.length];
...
function buildDemoAppointments(patientRefs, today) {
  const agenda = [];
  ...
  for (let offset = -7; offset <= 21; offset += 1) {
    ...
    for (let i = 0; i < maxSlots; i += 1) {
      ...
      patientCursor += 1;
      const noteIndex = Math.abs(offset * 3 + i + patientCursor) % APPOINTMENT_NOTES.length;
      ...
      agenda.push({
        ...
        meetingLink: APPOINTMENT_SEED_MARKER,
        notes: APPOINTMENT_NOTES[noteIndex],
      });
    }
...
async function ensureSecretaries(clinicId, doctorId) {
        ...
        `
          ...
          SET
            ...
            preferences = (
              ...
              jsonb_build_object('delegateForUserId', $7::int, 'delegateType', 'secretary', 'appStoreDemo', true)
            )::json
          WHERE id = $8
        `,
...
async function upsertPatients(doctorId, clinicId) {
...
async function seedClinicalContext(doctorId, patientRefs) {
  const profileIds = patientRefs.map((p) => p.id);
  await pool.query('DELETE FROM diagnoses WHERE user_id = $1 AND profile_id = ANY($2::int[])', [doctorId, profileIds]);
  await pool.query('DELETE FROM allergies WHERE user_id = $1 AND profile_id = ANY($2::int[])', [doctorId, profileIds]);
  ...
  for (let i = 0; i < patientRefs.length; i += 1) {
    ...
    await pool.query(
      ...
      `,
      [doctorId, patient.id, cidCode, diagnosisDate, diagnosisStatus, 'Diagnostico registrado para acompanhamento clinico.']
    );
    ...
    await pool.query(
      ...
      `,
      [doctorId, patient.id, allergy.allergen, allergy.reaction, allergy.severity, 'Alergia cadastrada para seguranca do atendimento.']
    );
  }
}

async function seedAgenda(doctorId, clinicId, patientRefs) {
  const profileIds = patientRefs.map((p) => p.id);
  await pool.query(
    `
    ...
    [
      doctorId,
      APPOINTMENT_SEED_MARKER,
      profileIds,
      `%${LEGACY_VISIBLE_TAG}%`,
      '%Consulta ficticia para vitrine da App Store%',
      '%Bloqueio automatico de agenda%',
    ]
  );

  const today = new Date();
  const appointments = buildDemoAppointments(patientRefs, today);
...
async function main() {
  ...
  const [profilesCount, appointmentsCount, secretariesCount] = await Promise.all([
    pool.query('SELECT COUNT(*)::int AS count FROM profiles WHERE user_id = $1 AND email LIKE $2', [doctorId, `%${DEMO_EMAIL_SUFFIX}`]),
    pool.query('SELECT COUNT(*)::int AS count FROM appointments WHERE user_id = $1 AND meeting_link = $2', [doctorId, APPOINTMENT_SEED_MARKER]),
    pool.query(
      `
