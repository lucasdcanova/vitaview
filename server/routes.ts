...
const syncUserSubscriptionState = async ({
  ...
  if (
    existingSubscription?.appStoreOriginalTransactionId &&
    appStoreOriginalTransactionId
  ) {
    const [existingPlan, incomingPlan] = await Promise.all([
      existingSubscription.planId
        ? storage.getSubscriptionPlan(existingSubscription.planId)
        : Promise.resolve(undefined),
      storage.getSubscriptionPlan(planId),
    ]);

    const shouldReplace = shouldReplaceAppStoreSubscription({
      existingAppleProductId: existingPlan?.appleProductId,
      existingPlanName: existingPlan?.name,
      existingOriginalTransactionId:
        existingSubscription.appStoreOriginalTransactionId,
      existingStatus: existingSubscription.status,
      existingCurrentPeriodEnd: existingSubscription.currentPeriodEnd,
      incomingAppleProductId: incomingPlan?.appleProductId,
      incomingPlanName: incomingPlan?.name,
      incomingOriginalTransactionId: appStoreOriginalTransactionId,
      incomingCurrentPeriodEnd: currentPeriodEnd,
    });
